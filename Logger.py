...
class Logger():
    ...
    def setup_CPU_before_train(self, algo, job_id: str):
        self.update_column("time", lambda: datetime.now())
    ...
    def setup_CPU_after_train(self, algo, job_id: str):
        self.update_column("time", lambda: datetime.now())
        self.update_column("gen", lambda: algo.event_name_to_data('')['generation_num'])
        self.update_column("type", lambda: "CPU")
        self.update_column("measure", lambda: str(subprocess.check_output(["sstat", "-j" + job_id, "-a", "--format=ConsumedEnergyRaw"])).split("\\n")[-2].strip())
    ...
    def setup_evolution_statistics(self, algo):
        ...
        self.update_column("best_of_run", lambda: algo.get_individual_evaluator().evaluate_individual(algo.best_of_run_) if algo.best_of_gen is not None else 0)
